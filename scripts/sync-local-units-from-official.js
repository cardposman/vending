...
const manualPreserveDistricts = new Set([
  '서울특별시 은평구'
]);
...
function slugForUnit(name, used) {
  const base = unitSlugOverrides[name] || romanizeText(name)
    .replace(/[^a-z0-9-]/g, '-')
    .replace(/-+/g, '-')
    .replace(/^-|-$/g, '');
  let slug = base || 'local';
...
function hasChildDistrict(district, districts) {
  return districts.some((candidate) => candidate.officialName.trim().startsWith(`${district.officialName.trim()} `));
}
...
function buildUnitCopy(district, unitName, allUnitNames, index) {
  const seed = hash(`${district.officialName} ${unitName}`);
  const neighbors = nearbyNames(allUnitNames, index);
  ...
  const leads = [
    ...
    `${unitName} 주변은 반복 방문 수요와 매장 체류 시간이 달라질 수 있어 운영 상품과 보충 동선을 먼저 나눠 보는 편이 좋습니다.`
  ];
  const contexts = [
    `${unitName}에서는 고객 대기 공간, 직원 복지 공간, 생활편의 판매 위치를 나눠 보면 필요한 자판기 온도 조건과 결제 방식을 잡기 좋습니다.`,
    `${district.districtName} 내 다른 생활권과 비교해 ${unitName} 일대는 매장 규모, 출입 동선, 관리 가능 시간에 맞춘 구성이 중요합니다.`,
  ...
  const nearbys = [
    ...
    `${unitName} 상담에서는 ${focusText}을 기준으로 고객 대기 공간, 직원 복지 공간, 생활편의 판매 위치를 구분해 볼 수 있습니다.`
  ];
  return {
...
function main() {
  ...
  const province = provinceArg?.split('=')[1] || 'seoul';
  ...
  regions.districts = regions.districts.map((district) => {
    if (district.provinceSlug !== province && district.provinceName !== province && district.provinceOfficialName !== province) return district;
    if (hasChildDistrict(district, regions.districts)) {
      skippedParents += 1;
      return district;
    }
    const names = localUnitNamesForDistrict(district, legalRows);
    if (!names.length) return district;

    const existingByName = new Map((district.dongs || []).map((unit) => [unit.name, unit]));
    const usedSlugs = new Set();
    const dongs = names.map((name, index) => {
      const existing = existingByName.get(name);
      const slug = existing?.slug || slugForUnit(name, usedSlugs);
      if (existing?.slug) usedSlugs.add(existing.slug);
      if (existing && manualPreserveDistricts.has(district.officialName)) {
        preservedUnits += 1;
        return existing;
      }
      if (!existing) createdUnits += 1;
      return {
        name,
        slug,
        ...buildUnitCopy(district, name, names, index)
      };
    });
  ...
  console.log(`Synced local units for ${province}: ${touchedDistricts} districts.`);
  console.log(`Skipped ${skippedParents} parent districts with child districts.`);
  console.log(`Created ${createdUnits} units, preserved ${preservedUnits} manual units.`);
}
