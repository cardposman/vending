...
이 문서는 `vending-test`를 새 GitHub 저장소로 분리한 뒤에도 같은 기준으로 작업을 이어가기 위한 인수인계 문서입니다.

## 프로젝트 개요

- 사이트명: TS밴딩시스템
- 상품명: 자판기렌탈
- 현재 위치: 기존 POS 사이트 내부의 테스트 폴더 `vending-test`
- 권장 이전 방식: 새 저장소에서는 `vending-test` 안의 내용을 저장소 루트로 이동
- 현재 색인 정책: 모든 테스트 페이지는 `noindex,nofollow`
- 현재 sitemap 정책: 기존 POS 사이트의 `sitemap.xml`에는 `/vending-test/` URL을 넣지 않음
...
```text
/vending-test/
/vending-test/region/seoul/eunpyeong-gu/
/vending-test/region/seoul/eunpyeong-gu/{dong-slug}/
```
...
즉 새 저장소에서는 HTML, CSS, JS 안의 `/vending-test/` 경로를 `/` 기준 경로로 바꿔야 합니다.
...
- `/vending-test/` 경로를 `/` 기준으로 변경
- canonical을 새 도메인 기준으로 변경
- `og:url`을 새 도메인 기준으로 변경
- `og:image`을 새 도메인 기준 절대 URL로 변경
- `noindex,nofollow` 제거
- `robots.txt` 생성 또는 수정
- `sitemap.xml` 생성
- 생성된 전체 URL 목록 파일 저장
...
1. 새 GitHub 저장소 생성
2. 현재 `vending-test` 폴더 안의 내용을 새 저장소 루트로 복사
3. `PROJECT_GUIDE.md`도 함께 복사
4. `/vending-test/` 경로를 `/` 기준으로 변경
5. 은평구 샘플 페이지 로컬 확인
6. 자판기 전용 지역 생성 스크립트 작성
7. 관리지역 기준으로 지역 확장
8. URL 목록 파일 생성
9. `noindex` 상태로 전체 검수
10. 도메인 연결 직전에 SEO 공개 전환 체크리스트 수행

## 현재 로컬 확인 방법

현재 기존 프로젝트 안에서 확인할 때:

```text
http://127.0.0.1:8787/vending-test/
```

새 저장소로 옮긴 뒤에는 로컬 서버 루트 기준으로 확인합니다.
